...
def model_to_dict(model: BaseModel) -> dict:
    ...
    assert isinstance(model, Model)
    for idx, submodel in enumerate(model.dag):
        model_id = f"m_{idx}"
...
def connection_to_dict(
    ...
    for key, connection in connections.items():
        ...
        if (
            ...
            # In case of extending from input and also exposed as output,
            # expose the key in the first pass.
            if (
                (key_name := connection.key) in model.output_keys
                and key in submodel.output_keys
                and key_name not in exposed_outputs
            ):
                key_value.setdefault("key", {}).setdefault("name", key_name)
                key_value["key"]["expose"] = True
                exposed_outputs.add(key_name)
